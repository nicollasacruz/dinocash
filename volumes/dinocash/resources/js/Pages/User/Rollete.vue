<template>
  <section id="section2" class="section2 h-screen mx-auto p-1 lg:p-10 flex items-center justify-center z-0" :style="{
    ...
    <div class="max-w-xl lg:max-w-2xl w-full">
      ...
      <div
        ...
        <img :src="pinoRoleta" class="absolute h-24 sm:h-28 md:h-32 lg:h-40 -top-2 lg:-top-6 w-[2px] md:w-[4px]"
          :style="`left: ${position}%`" />
        <div
          class="text-center flex flex-col items-center justify-center col-span-1 border-roxo border my-1 md:my-3 rounded-lg"
          v-for="({ label, complement }, i) in premios" :key="i">
          <div class="text-lg sm:text-2xl lg:text-3xl text-verde font-bold">
            {{ label }}
          </div>
          <div class="leading-none text-xs sm:text-sm md:text-base md:leading-tight text-white uppercase font-bold">
            {{ complement }}
          </div>
        </div>
      </div>
      <button :disabled="loading" @click="girarRoleta" class="user-button w-60 mx-auto">
        Girar
      </button>
    </div>
    <!-- Modal -->
    <BaseModal v-if="showModal" class="modal z-50">
      <div class="modal-content">
        <!-- Replicar a div onde o seletor parou -->
        <div class="replicar-div" v-if="selectedPremio">
          <div class="text-lg sm:text-2xl lg:text-3xl text-verde font-bold">
        ...
        <button @click="closeRoullete">Fechar</button>
      </div>
    </BaseModal>
  </section>
</template>
...
const postRecompensa = async (rewardOption) => {
  try {
    ...
    if (response.data.status === 'success') {
      ...
      selectedPremio.value = premios[rewardOption];
